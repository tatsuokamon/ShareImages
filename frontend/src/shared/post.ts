...
export async function get_initial_posts(
	...
	const img_promise = fetch(URLManager.get_posted_image_endpoint({ room_id }), {
		method: "GET",
		headers: {
	...
	const comment_promise = fetch(URLManager.get_posted_comment_endpoint({ room_id }), {
		method: "GET",
		headers: {
			Authorization: gen_authorization(token),
		},
	...
	const [imgs, comment] = await Promise.all([img_promise, comment_promise])
